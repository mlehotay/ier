...
8. emits:
   ...
   * a realized ordered path list (“booklist”) at the requested output path
   * optional diagnostic artifacts produced by the implementation

Pandoc consumes the emitted list **verbatim**.

---

## Dependency Artifacts and Order Verification

The build system may generate and verify **dependency-related artifacts** as part of build hygiene.

These steps validate **consistency** between:

* declared dependency metadata (chapter-local YAML), and
* the realized ordering produced by `extract_book_list.py`.

They do **not** infer content, modify ordering, or introduce interpretation.

### Dependency Generation

Dependency artifacts are generated by:

```
scripts/generate_deps.py
```

Inputs include:

* `IER-manifest.md` (authoritative chapter enumeration)
* a bundles configuration file (e.g., `assets/bundles.yml`)

Typical outputs include:

* derived dependency metadata (e.g., `build/dependencies.yml`)
* a non-authoritative audit artifact (e.g., `build/IER-prerequisites.md`)

Missing or incomplete dependency metadata may emit warnings or fail the build,
depending on generator configuration (e.g., strict metadata mode).

### Order Verification Against Dependencies

Ordering consistency with declared dependencies is verified by:

```
scripts/verify_order.py
```

This verifier checks that the realized ordering respects declared prerequisite
constraints using generated dependency metadata.

A build that violates dependency-order constraints is considered **invalid**,
even if ordering verification and Pandoc rendering succeed.
...
* dependency generation fails (when enabled)
* dependency-order verification fails (when enabled)
