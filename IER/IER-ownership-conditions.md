---
ier:
  tier: T2
  role: BRIDGE
  layer: pipeline
  domain:
  - agency_and_action
  category: agency_II
  status: canonical
  filename: IER-ownership-conditions.md
---
# Structural Authorship and Deformation Lineage in Frontier Geometry

## Structural Authorship and Deformation Lineage in Frontier Geometry

**Informational Experiential Realism (IER v10.9.5)**\
*Tier-2 — Explanatory — Non-Normative — Canon-Constrained*

## Status, Scope, and Authority

This document is explanatory and non-normative.

It:

* introduces no new ontological primitives
* does not redefine ownership, authorship, agency, collapse, or propagation
* does not introduce indeterminism, probabilistic branching, or selectors
* does not create new collapse types
* does not revise the resolution pipeline
* does not establish moral ranking or criteria

All ontological and structural authority remains fixed exclusively in:

The relevant canonical IER documents govern this clarification.

If any statement here conflicts with Tier-1 commitments, Tier-1 prevails.

Purpose:

> To clarify how ownership phenomenology varies structurally depending on the lineage of deformation shaping frontier geometry at collapse.

This document introduces no new machinery.

## Ownership Is Invariant

Under IER:

* Intrinsic constraint is borne at a single history — future boundary.
* Collapse is atomic and exclusive.
* All collapse at the frontier is owned.

Ownership does not scale.
Ownership does not weaken.
Ownership does not admit degrees.

If collapse occurs at a Unified Experiential Field (UEF), it is borne there.

There are not two kinds of collapse:

* not “internal” collapse
* not “external” collapse

There is only:

> Atomic collapse at an owned frontier.

What varies is not ownership itself.

What varies is:

> The deformation lineage shaping the geometry in which collapse occurs.

## Frontier Geometry and Collapse Conditions

At each frontier state:

* Local viability geometry encodes anisotropic fragility.
* State evolution reshapes that geometry continuously.
* Collapse occurs when continued multiplicity becomes structurally unsustainable under intrinsic constraint.

(For formal encoding of local geometry, see *IER-frontier-geometry*.)

Collapse is determined by total deformation present at that frontier state.

No indeterminacy gap exists.

The geometry is fully determined by:

* prior welded deformation
* propagation
* ongoing coupling
* current structural configuration

Ownership concerns where collapse is borne.
Ownership conditions concern how the geometry producing collapse was shaped.

## Deformation Lineage

Frontier geometry is never uncaused.

It is shaped by deformation lineage.

Deformation lineage includes:

* Prior atomic collapses within the same UEF.
* Welding and propagation of earlier irreversible exclusions.
* Ongoing coupling with other systems and environments.

All of these lawfully contribute to present fragility geometry.

No additional field is required.

No force is required.

Only:

* atomic collapse
* welded deformation
* propagation
* continuous traversal

Ownership conditions describe how this lineage distributes.

## Internally Dominated Foreclosure

In some collapse events:

* Present frontier geometry is shaped predominantly by prior atomic collapses within the same UEF.
* Fragility gradients reflect long-horizon internal deformation.
* External coupling influences but does not dominate geometry.

Collapse remains fully determined.

Ownership remains invariant.

But deformation lineage is primarily internal.

Structurally:

> Present admissible-future geometry depends chiefly on prior owned collapse within the same regime.

Phenomenologically, such cases often correspond to:

* strong authorship
* coherent self-boundary
* high integration
* “I did this”

No chooser is involved.

Only internal deformation dominance.

## Externally Dominated Foreclosure

In other collapse events:

* Present frontier geometry is sharply reshaped by strong external coupling.
* Switching costs are rapidly altered by imposed constraint.
* Fragility asymmetry reflects deformation lineage heavily influenced by external systems.

Collapse remains atomic.
Ownership remains intact.

But deformation lineage is predominantly external.

Structurally:

> Present admissible-future geometry depends primarily on deformation introduced through external coupling rather than prior internally welded exclusion.

Phenomenologically, such cases may correspond to:

* coercion
* invasion
* constraint imposition
* “this happened to me”

Ownership does not disappear.

Authorship lineage shifts.

## Mixed Regimes

Most collapse events occur in mixed regimes.

Frontier geometry is shaped by:

* sedimented internal collapse lineage
* external coupling
* ongoing propagation

Ownership phenomenology reflects:

> The relative dominance of internal versus external deformation lineage in shaping collapse geometry.

This is not a quantitative scale.

It is a structural configuration of lineage within fragility geometry.

## No Indeterminacy, No Chooser

This document does not introduce:

* symmetric possibility spaces
* underdetermined collapse points
* libertarian freedom
* stochastic selection
* a chooser filling structural slack

At every frontier state:

> Exactly one continuation is structurally sustainable.

Ownership conditions do not introduce metaphysical freedom.

They describe deformation authorship within a fully determined geometry.

## Relation to Free Will

Under IER:

> Free will is owned narrowing under intrinsic constraint.

Ownership conditions refine this by clarifying:

* how deformation shaping narrowing originates
* how coupling reshapes fragility gradients
* why some collapses feel agentive while others feel imposed

Free will remains:

* non-libertarian
* non-probabilistic
* fully geometrically determined

Ownership variation does not imply indeterminism.

It describes lineage dominance.

## Relation to Authorship

From *IER-authorship*:

> Authorship is the diachronic dependence of present frontier geometry on prior atomic collapses welded under ownership within the same UEF.

Ownership conditions specify:

* whether present geometry depends primarily on prior internally welded collapse
* or whether deformation shaping geometry is largely introduced via external coupling

Authorship may remain structurally intact even when phenomenological ownership feels diminished.

Ownership conditions concern deformation lineage dominance, not narrative self-ascription.

## Structural Summary

* Collapse is atomic and exclusive.
* Ownership is invariant.
* Frontier geometry is fully determined by deformation lineage.
* Deformation lineage includes internal collapse history and external coupling.
* Internally dominated lineage produces high authorship coherence.
* Externally dominated lineage produces ownership strain without ownership loss.
* No chooser is required.
* No indeterminacy gap exists.

> Ownership never varies.
> Collapse never splits.
> What varies is the lineage of deformation shaping the geometry in which collapse becomes inevitable.

Nothing more is added.
