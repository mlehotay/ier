...
Information refers only to physically instantiated distinctions in admissible transition structure. It does not refer to content, representation, or message.
...
### Physical Information (Derived)

*Information* is not a primitive.

A system is described as *informational* $\text{iff}$ its physically instantiated state distinctions modulate the system’s own future trajectories under constraint.
...
### System Boundary (for UEF Attribution)
...
The world-for-the-system that exists $\text{iff}$ a UEF exists.
...
* exists $\text{iff}$ experience exists
...
* Too little constraint $\to$ no experience
* Excessive or incoherent constraint $\to$ fragmentation or collapse
...
A system has standing $\text{iff}$ it instantiates a UEF.
...
Experience may be described as a physical regime in which intrinsic constraint is globally unavoidable, internally borne, temporally continuous, and self-sustaining, producing a system-relative world organized by structured tension.

This restatement introduces no additional criteria and is equivalent to the definition of a Unified Experiential Field above.
